...
export const load: PageLoad = ({ params, url }) => {
    ...
    const postsBeratung = fetch(
        `https://blog.triarc-labs.com/ghost/api/content/posts?include=tags,authors&key=93ed4aea5970c22ed269d4ec35&limit=10&order=published_at%20desc`
    )
    ...
    return Promise.all([ postsBeratung]).then(
        async ([postResponse]) => {
