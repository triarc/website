<script lang="ts">
  let firstName = '', lastName = '', phone, email = '', subject = '', message = ''
  let sent = false;
  let sending = false;
            ...
            <div class="sm:col-span-2 sm:flex sm:justify-end space-between gap-x-6">
              {#if sent}
                <div class="rounded-md bg-green-triarc bg-opacity-10 p-4 mt-2 flex-grow">
                  <div class="flex">
                    <div class="ml-3">
                      <p class="text-sm font-medium text-green-800">Deine Nachricht wurde übermittelt, wir melden uns so rasch wie möglich</p>
                    </div>
                  </div>
                </div>
              {/if}
              ...
              <button
                on:click={send} disabled="{sending || !firstName || !lastName || !subject || !message}"
                type="button"
