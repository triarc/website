            ...
            <dt class="text-lg">
              <button class="text-left w-full flex justify-between items-start text-gray-400 focus:outline-none">
                ...
                <span class="ml-6 h-7 flex items-center">
                  <svg class="{ currentOpenAccordion === id ? '-rotate-180' : 'rotate-0' }  h-6 w-6 transform text-green-triarc" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                  </svg>
                </span>
              </button>
            </dt>
            <dd class="mt-2 pr-8 transform transition-transform duration-500 { currentOpenAccordion === id ? 'translate-y-0 text-gray-500' : '-translate-y-12 text-transparent h-0' }" >
              {#each content as p }
